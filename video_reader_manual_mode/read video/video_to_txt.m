function video_to_txt(vdeo_name, C1, C2)

v = VideoReader(vdeo_name);
fileID = fopen(strcat(vdeo_name(1:end-4), '.txt'), 'wt');
time_start = "00:00 s"; 
op_left_last = ''; 
op_right_last = ''; 
% swap_situation_last = 'Mini-map';

while hasFrame(v)
    frame = readFrame(v);
    if ~isequal(reshape(frame(500, 400, :), 1, 3), [237 237 237])
        op_left_last = ''; op_right_last = ''; 
        continue;
    end
    txt_time = ocr(frame(430:470,550:670));
    time = strtrim(string(txt_time.Text));
%     flag_print = 0;
    
    if time == time_start
        continue;
    end        
    
    I = rgb2gray(frame);
    [center_left] = imfindcircles(I(1:300, 100:300),[25 35], 'ObjectPolarity','dark', 'Sensitivity', 0.9);
    [center_right] = imfindcircles(I(1:300, 800:1100),[25 35], 'ObjectPolarity','dark', 'Sensitivity', 0.9);
    
    if ~isempty(center_left)
        d1 = pdist([C1; center_left]);
        if d1 > 10
            if center_left(2)- C1(2) < -5
                op_left = 'Rise';
            elseif center_left(2)- C1(2) > 5
                op_left = 'Descent';
            elseif center_left(1)- C1(1) < -5
                op_left = 'Rotate Left';
            elseif center_left(1)- C1(1) > 5
                op_left = 'Rotate Right';
            end
        else
            op_left = '';
        end
    else
        op_left = '';
    end
    
    if ~strcmp(op_left, op_left_last) && ~strcmp(op_left, '')
        fprintf(fileID, '%s\n', op_left);
%         flag_print = 1;
    end
    
    if ~isempty(center_right)
        d2 =pdist([C2; center_right]);
        if d2 > 10
            op_right = get_direction(center_right, C2);
%             op_right = num2str(degree);
        end
    else
        op_right = '';
    end
    
    if ~strcmp(op_right, op_right_last) && ~strcmp(op_right, '')
        fprintf(fileID, '%s\n', op_right);
%         flag_print = 1;
    end
    
%     if isequal(reshape(frame(400, 400, :), 1, 3), [215 215 215])
%         swap_situation = 'Mini-map';
%     else
%         swap_situation = 'Main Camera';
%     end 
    
%     if ~strcmp(swap_situation, swap_situation_last)
%         fprintf(fileID, 'View Switching\n');
% %         flag_print = 1;
%     end
    
%     if flag_print == 1
%         fprintf(fileID, '\n');
%     end
    
    op_left_last = op_left; 
    op_right_last = op_right; 
%     swap_situation_last  = swap_situation;
end

fclose(fileID);
